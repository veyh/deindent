...
command_help(command_t *self) {
  ...
  for (i = 0; i < self->option_count; ++i) {
    ...
    if (option->small && option->large_with_arg) {
      printf("    %s, %-25s %s\n"
        , option->small
    ...
    else if (!option->small && option->large_with_arg) {
      printf("        %-25s %s\n"
        , option->large_with_arg
        , option->description);
    }
...
command_parse_args(command_t *self, int argc, char **argv) {
  ...
  for (i = 1; i < argc; ++i) {
    ...
    for (j = 0; j < self->option_count; ++j) {
      ...
      // match flag
      if ((option->small != NULL && !strcmp(arg, option->small))
      ||  (option->large != NULL && !strcmp(arg, option->large))) {
        self->arg = NULL;
